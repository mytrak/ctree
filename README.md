...
Replicate any Docker-compose based dev tree as a sibling git worktree,
with its own branch, its own docker volumes, and a tailored `.env` —
so multiple branches can run side by side without thrashing each other's
state.
...
```bash
git clone {{GITHUB_HANDLE}}/ctree.git ~/Documents/ctree
```

`ctree shell-init` prints a shell function; you `eval` its output so the
function is defined in your current shell. Add this line to your
**`~/.zshrc`** (zsh, the macOS default) or **`~/.bashrc`** (bash). Don't put
it in `~/.bash_profile` or `~/.zprofile` — those run only for login shells,
but the function needs to be defined for every interactive shell.

```bash
eval "$($HOME/Documents/ctree/bin/ctree shell-init)"
```
...
- **`parse error` from the `eval` line** — run the inner command on its
  own (`$HOME/Documents/ctree/bin/ctree shell-init`) and confirm it
  prints a shell function. If the path is wrong the binary won't run and
  `eval` receives an error message instead of a function.
...
case. The preview shown before the `yes` prompt indicates whether a
same-named branch exists — clean up orphans manually with:
...
# during `ctree create` and `ctree update`. Use for gitignored artefacts
# that must match the source after a volume sync (e.g. lock files next to
# gem volumes). Files are copied directly; directories are synced
# recursively (additions, modifications, and deletions).
update:
  - Gemfile.rails71.plugins.lock   # a gitignored lock file
  - tmp/cache/shared               # a directory — synced recursively
...
# The TLD appended to the worktree name to form HOST_NAME_SUFFIX in the
# worktree .env (e.g. docker → HOST_NAME_SUFFIX=docker). Leave empty to
# disable. See "Traefik / reverse-proxy domain routing" below.
host_name_suffix: ""

# The docker-compose override file ctree appends to the worktree's
# COMPOSE_FILE and rewrites shared-volume entries in. Default shown.
...
# worktree is available. Default shown.
...
`ctree create` automatically replicates the entire source directory —
tracked files AND gitignored content (node_modules, build artifacts,
plugin lock files, etc.) — so no per-repo path configuration is needed.
See **add** above for how the cloning works.

### How to configure

1. Scaffold an annotated starting file by running, from anywhere inside
   your source repo:
...
2. Edit the keys you want to change, and **remove the keys you want to
   ...
   node_modules and also syncs a lock file and a cache directory:
